...
save_path = f'../pycharm_project_916/experiments/masked-{args.model}-{args.dataset}-{args.n_channels}'
...
class MaskedTrainer:
    ...
    def load_best_model(self,task_id):
        load_path = '{}/best_epoch_{}.pt'.format(save_path, task_id)
    ...
    def train_task(self, task_id):
        ...
        if task_id == 0:
            ...
            prev_task = self.train_sequence[task_id - 1]
            load_path = '{}/best_epoch_P{}.pt'.format(save_path, task_id)
            model_state_dict = torch.load(load_path)["model_state_dict"]
            model = choose_model().to(device)
            model.load_state_dict(model_state_dict)
            print(f"Loaded model from {prev_task} for fine-tuning")
        if task_id > 1:
            if task_id == 3:
                args.lr = args.lr * 0.4
            args.lr = args.lr * 0.4
        ...
        for epoch in range(self.args.joint_epochs):
            ...
            if val_mse < self.best_metrics[task_name]["mse"]:
                ...
                torch.save({
                    ...
                    'val_loss': val_mse
                }, f"{save_path}/best_epoch_{task_name}.pt")
