...
class SpatialMask(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        assert H % self.patch_size == 0 and W % self.patch_size == 0,"Size must be divisible by patch_size"
...
class UNO(nn.Module):
    def __init__(self, height=32, width=32, use_exf=False, scale_factor=4,
        ...
        self.config.patches.grid = (int(32 / 16), int(32 / 16))
        self.con_v = VisionTransformer(self.config, backbone=L2HNet(width=64, image_band=3, length=5,
                 ratios=[1, 0.5, 0.25],
                 bn_momentum=0.1), img_size=32,
                                       num_classes=64).cuda()
